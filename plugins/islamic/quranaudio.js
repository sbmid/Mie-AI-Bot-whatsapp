...
module.exports = {
    command: ['listsurah', 'listsurat', 'quranaudio', 'alquranaudio'],
    handler: async (sock, m, { text, prefix, command }) => {
        const from = m.chat;
        const num = text.trim();
        
        const surahList = `*LIST SURAH AL-QUR'AN*

\`Ingin mendengar Surah nya?\`
> # Ketik *${prefix}quranaudio* <nomor surah nya>
> *Contoh:* ${prefix}quranaudio 1
...
114. An-Nas (Umat Manusia)`;
        ...
        if (command.startsWith('list')) {
            return sock.sendMessage(from, {
                image: { url: "https://files.catbox.moe/f5e50t.jpg" },
                caption: surahList
            }, { quoted: m });
        }
        ...
        try {
            const { data } = await axios.get('https://raw.githubusercontent.com/dcode-al/database/refs/heads/main/Islami/quranaudio.json');
            const tod = data[num - 1];

            if (!tod) {
                return sock.sendMessage(from, { text: '[!] Nomor surah tidak ditemukan.' }, { quoted: m });
            }
...
_Loading Audio..._`;
            ...
            await sock.sendMessage(from, { text: quranText }, { quoted: m });
            ...
            console.error(error);
            sock.sendMessage(from, { text: '[!] Terjadi kesalahan' }, { quoted: m });
        }
    }
};
