...
module.exports = {
    command: ['ramalanjodohbali', 'jodohbali'],
    category: ['primbon'],
    handler: async (sock, m, { text, prefix, command }) => {
        ...
        try {
            ...
            const { data } = await axios.get(`https://api.siputzx.my.id/api/primbon/ramalanjodohbali?nama1=${encodeURIComponent(nama1)}&tgl1=${tgl1}&bln1=${bln1}&thn1=${thn1}&nama2=${encodeURIComponent(nama2)}&tgl2=${tgl2}&bln2=${bln2}&thn2=${thn2}`);
            
            if (data.status && data.data) {
                const res = data.data;
                let reply = `* RAMALAN JODOH BALI *\n\n`;
                reply += `[!] *Nama Anda:* ${res.nama_anda.nama}\n`;
                reply += `Lahir: ${res.nama_anda.tgl_lahir}\n\n`;
                reply += `[!] *Nama Pasangan:* ${res.nama_pasangan.nama}\n`;
                reply += `Lahir: ${res.nama_pasangan.tgl_lahir}\n\n`;
                
                reply += `*Hasil Result:*\n`;
                reply += `${res.result.replace(/\*/g, '\n*')}\n\n`;
            ...
            console.error("Ramalan Jodoh Bali Error:", e);
