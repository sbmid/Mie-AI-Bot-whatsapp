...
async function createBratVideo(text, outputPath, config) {
  ...
  const frames = buildRevealFrames(inputText, config);
  if (!frames.length) throw new Error("Teks kosong");
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "vermile-bratvid-"));
...
module.exports = {
  command: ['bratvermeil', 'bratvidvermeil'],
  handler: async (sock, m, { text, prefix, command }) => {
    ...
    try {
      ...
      const outputPath = path.join(tempDir, `vermeil-bratvid-${Date.now()}.mp4`);

      await createBratVideo(text, outputPath, VIDEO_CONFIG);
