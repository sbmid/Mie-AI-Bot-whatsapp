...
async function generateBratVideo({
  ...
  const tempAssetsDir = path.join(process.cwd(), 'temp')
  if (!existsSync(tempAssetsDir)) require('fs').mkdirSync(tempAssetsDir, { recursive: true })
  
  const outPath = path.join(tempAssetsDir, `bratvid2-${Date.now()}.${ext}`)
