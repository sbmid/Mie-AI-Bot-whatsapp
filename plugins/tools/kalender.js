...
function renderTahun(outputPath) {
  daftarkanFont();
...
function renderBulan(bulanNum, outputPath) {
...
module.exports = {
  ...
  handler: async (sock, m, { args, prefix, command }) => {
    try {
      ...
      let outPath = "";
      let reqBulan = args[0] ? args[0].toLowerCase() : null;

      if (reqBulan && NAMA_KE_NOMOR[reqBulan]) {
        const nomor = NAMA_KE_NOMOR[reqBulan];
        outPath = path.join(tempDir, `kalender-2026-${reqBulan}-${Date.now()}.png`);
        renderBulan(nomor, outPath);
      } else {
        outPath = path.join(tempDir, `kalender-2026-full-${Date.now()}.png`);
        renderTahun(outPath);
      }

      await sock.sendMessage(m.chat, { image: { url: outPath }, caption: "Ini kalendernya Kak!" }, { quoted: m });
