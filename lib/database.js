...
module.exports = {
    ...
    loadDatabase: async () => {
        try {
            ...
            if (e.message && e.message.includes('fetch failed')) {
                console.log("⚠️ [SUPABASE] Sedang offline atau gagal fetch, menggunakan local cache sementara.");
            } else {
    ...
    saveAll: async () => {
        if (!isLoaded) return;
        const usersArray = Object.values(db.users).filter(u => u && u.jid).map(u => ({
            jid: u.jid,
            name: u.name,
            xp: u.xp,
            level: u.level,
        ...
        const groupsArray = Object.values(db.groups).filter(g => g && g.jid).map(g => ({
            jid: g.jid,
            welcome: g.welcome || false,
            listrs: db.chats?.[g.jid]?.listrs || {}
        }));
        const sewaArray = Object.values(db.sewa || {}).filter(s => s && s.jid).map(s => ({
            jid: s.jid,
            expired: s.expired || 0,
